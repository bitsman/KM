...
5. 用完后卸载监听器：
   ...
   #+end_src
** hal
本小结主要参考[[https://blog.csdn.net/luoshengyang/article/details/6567257][CSDN]]博客。
...
android从下到上涉及到了android系统多层，如图[[fig:android-frmwk]]所示。
#+CAPTION: android框架
#+NAME: fig:android-frmwk
#+attr_latex: :placement [H] :width 0.6\textwidth
[[./img/img-android-frmwk.png]]
...
#+BEGIN_SRC shell -n 1 :exports code
...
#+BEGIN_SRC c -n 1 :exports code
  ...
  {
      int val;//代表设备里面的寄存器
      struct semaphore sem;//信号量，同步对val的访问
      struct cdev dev;//表示该设备是内嵌的字符设备
  };
  #endif
#+END_SRC
...
- devfs方式
  ...
  #+BEGIN_SRC c +n :exports code
    ...
    static ssize_t hello_val_show(struct device* dev,
                                  ...
                                  char* buf)
    {
        struct hello_android_dev* hdev =
            (struct hello_android_dev*)dev_get_drvdata(dev);        
...
- proc方式
  ...
  #+BEGIN_SRC c +n :exports code
    ...
    static ssize_t hello_proc_read(char* page,
                                   char* * start,
                                   off_t off,
                                   int count,
                                   int* eof,
    ...
    static ssize_t hello_proc_write(struct file* filp,
...
- 测试文件
  ...
  #+BEGIN_SRC c -n 1 :exports code
    ...
    int main(int argc, char** argv)
    {
        ...
        int val = 0;
        // 打开/dev/hello文件
        fd = open(DEVICE_NAME, O_RDWR); //系统调用
        if(fd == -1)
        {
        ...
        //先读出/dev/hello文件中的值
        read(fd, &val, sizeof(val)); //系统调用
        printf("%d.\n\n", val);
        val = 5;
        printf("Write value %d to %s.\n\n", val, DEVICE_NAME);
        //写入值5到/dev/hello中去
        write(fd, &val, sizeof(val)); //系统调用
...
- 首先是包含相关头文件和定义相关结构
  #+BEGIN_SRC c -n 1 :exports code
    ...
    {//通过该方法打开/dev/hello设备文件，用malloc做内存映射，
     //就可以访问该驱动文件的各种方法和属性
    ...
    {
      common: {
          tag: HARDWARE_MODULE_TAG,//hal规定该名称固定
          version_major: 1,
          version_minor: 0,
          id: HELLO_HARDWARE_MODULE_ID,//hal规定该名称固定
          name: MODULE_NAME,
          author: MODULE_AUTHOR,
          methods: &hello_module_methods,//上面的方法表
      }
    };
  #+END_SRC
...
**** hal构建
- mk文件
  继续在hello目录下新建Android.mk文件, LOCAL_MODULE的定义规则，hello
...
void *pvPortMalloc( size_t xWantedSize )
{
  ...
  #+end_src
- heap\_2.c: 同heap\_1.c差不多，只是采用一定算法来分配空间，增加了堆释放操作, ；
#+begin_src c
...
#+end_src
- heap\_4.c: 同heap\_2.c差不多，只是分配算法做了更优化，并且相邻空闲空间可以合并；
#+begin_src c
void *pvPortMalloc( size_t xWantedSize )
{
