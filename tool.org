...
* jenkins
- 单个git库被触发的设置：
  1. 保证git服务器和Jenkins服务器网络通畅；
  2. 在该git库的.git/hooks/中新建post-receive脚本
     #+BEGIN_SRC shell
       #!/bin/sh
     ...
     #+END_SRC
  3. 这样对该git库提交，就会触发该job
- 批量git库设置触发：
  1. 保证git服务器和Jenkins服务器网络通畅；
  2. 进入git服务器的 ~/.gitolite/hooks/common，编辑post-receive
     #+BEGIN_SRC shell
       #!/bin/sh
       ...
       echo "done."
     #+END_SRC
  3. 运行命令gitolite setup。为每个git库建立软连接到该脚本，这样在做库
